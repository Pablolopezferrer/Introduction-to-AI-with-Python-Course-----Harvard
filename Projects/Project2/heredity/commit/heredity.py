...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["mother"] is None and people[person]["father"] is None:
            ...
            # Has parents, calculate probability based on parents' genes
            mother = people[person]["mother"]
            father = people[person]["father"]
            
            # Get mother's gene count
            ...
            # Calculate probability of inheriting from mother
            if mother_genes == 0:
                prob_from_mother = PROBS["mutation"]  # Only way is mutation
            elif mother_genes == 1:
                prob_from_mother = 0.5 * (1 - PROBS["mutation"]) + 0.5 * PROBS["mutation"]
            else:  # mother_genes == 2
                prob_from_mother = 1 - PROBS["mutation"]
                
            # Calculate probability of inheriting from father
            if father_genes == 0:
                prob_from_father = PROBS["mutation"]  # Only way is mutation
            elif father_genes == 1:
                prob_from_father = 0.5 * (1 - PROBS["mutation"]) + 0.5 * PROBS["mutation"]
            else:  # father_genes == 2
                prob_from_father = 1 - PROBS["mutation"]
                
            # Calculate gene probability based on gene count
            if gene_count == 0:
                gene_prob = (1 - prob_from_mother) * (1 - prob_from_father)
            elif gene_count == 1:
                gene_prob = prob_from_mother * (1 - prob_from_father) + (1 - prob_from_mother) * prob_from_father
            else:  # gene_count == 2
